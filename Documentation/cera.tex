...
\begin{itemize}
    ...
    \item \textbf{Interface Visibility Enforcement:} To guarantee a sound type system across module boundaries, the analyzer enforces an Interface Visibility Check during global registration. If a function is public (not marked \texttt{hidden}), the compiler strictly forbids its parameters or return type from referencing a \texttt{hidden} type. This mathematically prevents the creation of invalid type leaks where an external caller receives an opaque structure it cannot pattern-match or annotate.
\end{itemize}
