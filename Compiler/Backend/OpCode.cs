...
public enum OpCode : byte
{
    ...
    JUMP_IF_FALSE_PEEK = 0x53,
    JUMP_IF_TRUE_PEEK = 0x54,

    // --- Fused Relational Jumps
    JUMP_IF_FALSE_EQ = 0x55,
    JUMP_IF_FALSE_NEQ = 0x56,
    JUMP_IF_FALSE_LT = 0x57,
    JUMP_IF_FALSE_LTE = 0x58,
    JUMP_IF_FALSE_GT = 0x59,
    JUMP_IF_FALSE_GTE = 0x5A,

    // --- Memory Fast-Paths ---
    JUMP_IF_LOCAL_NOT_EQ_CONST = 0x5B, // Consumes local slot and const index
    ...
    // Top level, wont attempt to capture upvalues
    CALL_GLOBAL = 0x65, 
    ...
    JUMP_IF_NOT_LIST_EMPTY = 0x84,  // Jump if non empty list
    JUMP_IF_NOT_ARRAY_LENGTH = 0x85, // Jump if array is not of expected length
